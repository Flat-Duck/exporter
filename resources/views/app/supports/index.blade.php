    ...
    <div class="table-responsive">
        <table class="table card-table table-vcenter text-nowrap datatable">
            <thead>
                <tr>
                    <th class="text-left">
                        @lang('crud.supports.inputs.description')
                    </th>
                    <th class="text-left">
                        @lang('crud.supports.inputs.support_type_id')
                    </th>
                    <th class="text-left">
                        @lang('crud.supports.inputs.exporter_id')
                    </th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ optional($support->supportType)->name ?? '-' }}</td>
                    <td>
                        {{ optional($support->exporter)->first_name ?? '-' }}
                    </td>
                    <td class="text-center" style="width: 134px;">
                        ...
                        >
                            ...
                            @endcan @can('view', $support)
                            <a
                                href="{{ route('supports.show', $support) }}"
                                class="btn btn-icon btn-outline-info ms-1"
                            >
                                <i class="ti ti-eye"></i>
                            </a>
                            @endcan @can('delete', $support)
                            <form
                                action="{{ route('supports.destroy', $support) }}"
                                method="POST"
